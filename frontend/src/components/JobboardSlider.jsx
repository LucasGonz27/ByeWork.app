    ...
    <section className={styles.sliderSection}>
      <div className={styles.headerRow}>
        ...
        <div className={styles.controls}>
          <button aria-label="Précédent" onClick={() => scrollByViewport(-1)} className={styles.navBtn}>←</button>
          <button aria-label="Suivant" onClick={() => scrollByViewport(1)} className={styles.navBtn}>→</button>
        </div>
      </div>
        ...
        <div className={styles.sliderTrack} ref={viewportRef}>
          {offers.map((offer) => (
            <article key={offer.idOffre || `${offer.nomEntreprise}-${offer.titre}`} className={styles.card}>
              ...
              <button className={styles.cta}>Voir l'offre</button>
            </article>
          ))}
        </div>
      </div>
